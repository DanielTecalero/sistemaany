...
export class RegistroMaestrosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.activatedRoute.snapshot.params['id'] != undefined){
      ...
      try {
        ...
        console.error("Error al parsear materias_json:", error);
        this.maestro.materias_json = [];
      }
    }

    // Si materias_json no existe o no es array, inicializarlo
  ...
  private cargarDatosUser(): void {
    if (this.datos_user && Object.keys(this.datos_user).length > 0) {
      this.maestro = {...this.datos_user}; // Copiar los datos
      ...
      this.MaestrosService.registrarMaestro(this.maestro).subscribe(
        ...
        (error) => {
          ...
          if (error.error && error.error.message) {
              ...
              alert("Error al registrar el maestro. Verifica los datos.");
          }
        }
      );
    }else{
