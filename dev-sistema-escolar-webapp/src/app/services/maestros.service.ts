...
export class MaestrosService {
  ...
  public validarMaestro(data: any, editar: boolean) {
    ...
    if (!this.validatorService.required(data["id_trabajador"])) {
      ...
      error["id_trabajador"] = this.errorService.alphanumeric;
    }

    //Nombre
    if (!this.validatorService.required(data["first_name"])) {
      error["first_name"] = this.errorService.required;
    } else if (!this.validatorService.max(data["first_name"], 40)) {
      error["first_name"] = this.errorService.max(40);
    ...
    } else if (!this.validatorService.max(data["last_name"], 60)) {
      error["last_name"] = this.errorService.max(60);
    ...
    } else if (!this.validatorService.notFutureDate(data["fecha_nacimiento"])) {
      error["fecha_nacimiento"] = this.errorService.notFutureDate;
    } else if (!this.validatorService.minAge(data["fecha_nacimiento"], 18)) {
      error["fecha_nacimiento"] = this.errorService.minAge;
    }
    ...
    } else if (!this.validatorService.cubiculoFormat(data["cubiculo"])) {
      error["cubiculo"] = this.errorService.cubiculoFormat;
    }
