...
class MaestrosView(generics.CreateAPIView):
    ...
    permission_classes = (permissions.IsAuthenticated,)
    def get(self, request, *args, **kwargs):
        maestros = get_object_or_404(Maestros, id=request.GET.get("id"), user__is_active=1)
        listaM = MaestroSerializer(maestros, many=False).data
        if isinstance(listaM, dict) and "materias_json" in listaM:
                try:
                    listaM["materias_json"] = json.loads(listaM["materias_json"])
                except Exception:
                    listaM["materias_json"] = []
        return Response(listaM, 200)
